...
export async function signupStep1 ({ fullName, username, email, password, phoneCountryCode, phoneNumber }) {
  ...
  const result = await withTransaction(async (client) => {
    const userRes = await client.query(
      `INSERT INTO public.users (full_name, username, email, hashed_password, phone_country_code, phone_number)
       VALUES ($1, $2, $3, $4, $5, $6)
    ...
    await client.query(
      `INSERT INTO public.phone_verifications (user_id, phone_country_code, phone_number, code_hash, expires_at)
...
export async function signupStep2 ({ userId, idNumber, frontIdUrl, backIdUrl }) {
  return withTransaction(async (client) => {
    const userRes = await client.query(
      'UPDATE public.users SET id_number = $1 WHERE user_id = $2 RETURNING user_id',
      [idNumber, userId]
    )
    ...
    await client.query(
      `INSERT INTO public.user_identity_documents (user_id, document_type, file_url)
...
export async function signupStep3 ({ userId, selfieUrl }) {
  return withTransaction(async (client) => {
    ...
    const result = await client.query(
      'SELECT account_status FROM public.users WHERE user_id = $1',
      [userId]
    )
...
export async function requestPhoneCode ({ userId }) {
  const user = await db.query(
    'SELECT user_id, phone_country_code, phone_number FROM public.users WHERE user_id = $1',
    [userId]
  )
...
export async function verifyPhone ({ userId, code }) {
  const result = await db.query(
    `SELECT phone_verification_id, code_hash, expires_at FROM public.phone_verifications
...
async function resetFailedAttempts (userId) {
  await db.query(
    'UPDATE public.users SET failed_login_attempts = 0, locked_until = NULL WHERE user_id = $1',
    [userId]
  )
}
...
export async function login ({ identifier, password }, { clientIp, userAgent, deviceName }) {
  ...
  await db.query(
    'UPDATE public.users SET last_login_at = NOW() WHERE user_id = $1',
    [user.user_id]
  )
...
export async function refreshTokens (refreshToken) {
  ...
  const sessionRes = await db.query(
    `SELECT session_id, user_id, refresh_token_hash, revoked_at, expires_at
     FROM public.user_sessions WHERE session_id = $1`,
    [decoded.sid]
  )
  ...
  const userRes = await db.query(
    'SELECT user_id, role, account_status FROM public.users WHERE user_id = $1',
    [session.user_id]
  )
...
export async function logout (sessionId) {
  await db.query(
    'UPDATE public.user_sessions SET revoked_at = NOW() WHERE session_id = $1',
    [sessionId]
  )
}
...
export async function changePassword (userId, currentPassword, newPassword) {
  const res = await db.query(
    'SELECT hashed_password FROM public.users WHERE user_id = $1',
    [userId]
  )
  if (res.rowCount === 0) throw createError(404, 'User not found')
  ...
  await db.query(
    `UPDATE public.users SET hashed_password = $1, password_changed_at = NOW()
     WHERE user_id = $2`,
    [hashed, userId]
  )
}
