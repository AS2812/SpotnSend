...
export async function listUsers (query) {
  ...
  const sql = `SELECT u.user_id, u.full_name, u.username, u.email, u.account_status, u.role, u.created_at,
       (SELECT COUNT(*) FROM public.reports r WHERE r.user_id = u.user_id) AS reports_count,
       (SELECT COUNT(*) FROM public.report_feedbacks rf WHERE rf.user_id = u.user_id) AS feedback_count
     FROM public.users u
               ${where}
  ...
  const countRes = await db.query(`SELECT COUNT(*) FROM public.users u ${where}`, params)
...
export async function updateUserStatus (userId, status) {
  ...
  `UPDATE public.users SET account_status = $2, updated_at = NOW()
...
export async function listPendingVerifications (query) {
  ...
  `SELECT av.verification_id, av.user_id, av.status, av.submitted_at, u.full_name, u.username, u.email
   ...
   JOIN public.users u ON u.user_id = av.user_id
  ...
  const countRes = await db.query(
    'SELECT COUNT(*) FROM public.account_verifications WHERE status = \'pending\''
  )
  const meta = buildPaginationMeta(Number(countRes.rows[0].count), { limit, page })
...
export async function reviewVerification (verificationId, reviewerId, { status, rejectionReason, notes }) {
  ...
  [verificationId, status, reviewerId || null, rejectionReason || null, notes || null]
    ...
    await client.query(
      'UPDATE public.users SET account_status = $2 WHERE user_id = $1',
...
export async function listAuditEvents (query) {
  ...
  const countRes = await db.query(`SELECT COUNT(*) FROM public.audit_events ${where}`, params)
  const meta = buildPaginationMeta(Number(countRes.rows[0].count), { limit, page })
  return { data: res.rows, meta }
}
