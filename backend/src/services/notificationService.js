...
export async function markNotificationsSeen(userId, ids) {
  ...
  const res = await db.query(
    `UPDATE civic_app.notifications
     SET seen_at = NOW()
...
export async function sendNotification({ userId, title, body, notificationType, payload, relatedReportId }, channels = ['in_app']) {
  return withTransaction(async (client) => {
    const notificationRes = await client.query(
      `INSERT INTO civic_app.notifications (user_id, notification_type, title, body, payload, related_report_id)
    ...
    if (channels && channels.length > 0) {
      ...
      await client.query(
        `INSERT INTO civic_app.notification_deliveries (notification_id, channel, status)
         VALUES ${placeholders}
