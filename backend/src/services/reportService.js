  ...
  return withTransaction(async (client) => {
    ...
    if (media.length > 0) {
      ...
      const insertMediaSql = format(
        'INSERT INTO civic_app.report_media (report_id, media_type, storage_url, thumbnail_url, metadata, is_cover) VALUES %L',
        values
      );
    ...
    const authoritiesRes = await client.query(
      `SELECT authority_id FROM civic_app.find_authorities_nearby($1, $2, $3, $4, $5)`,
      [
        latitude,
        longitude,
    ...
    if (authoritiesRes.rowCount > 0 && (notifyScope === 'government' || notifyScope === 'both')) {
      ...
      const sql = format(
        'INSERT INTO civic_app.report_authority_dispatches (report_id, authority_id, status, channel) VALUES %L ON CONFLICT (report_id, authority_id) DO NOTHING',
        values
      );
...
export async function getReport(reportId) {
  const res = await db.query(
    `SELECT ${reportFields},
     ...
     FROM civic_app.reports r
     JOIN civic_app.report_categories c ON c.category_id = r.category_id
     LEFT JOIN civic_app.report_subcategories sc ON sc.subcategory_id = r.subcategory_id
     JOIN civic_app.users u ON u.user_id = r.user_id
     WHERE r.report_id = $1`,
    [reportId]
  );
  ...
  const mediaRes = await db.query(
    `SELECT media_id, media_type, storage_url, thumbnail_url, metadata, is_cover, created_at
     FROM civic_app.report_media WHERE report_id = $1`,
    [reportId]
  );
  ...
  const feedbackRes = await db.query(
    `SELECT rf.feedback_id, rf.user_id, rf.feedback_type, rf.comment, rf.created_at,
            u.full_name
     FROM civic_app.report_feedbacks rf
     JOIN civic_app.users u ON u.user_id = rf.user_id
  ...
  const dispatchRes = await db.query(
    `SELECT rad.dispatch_id, rad.authority_id, a.name AS authority_name, rad.status,
     ...
     FROM civic_app.report_authority_dispatches rad
     JOIN civic_app.authorities a ON a.authority_id = rad.authority_id
...
export async function listUserReports(userId, query) {
  ...
  const countRes = await db.query(
    `SELECT COUNT(*) FROM civic_app.reports WHERE user_id = $1`,
    [userId]
  );
...
export async function getNearbyReports(params) {
  ...
  const result = await db.query(
    `SELECT * FROM civic_app.find_reports_nearby($1, $2, $3, $4, $5, $6, $7, $8)`,
    [
      params.latitude,
      params.longitude,
      params.radius,
...
export async function addFeedback(reportId, userId, { comment }) {
  const res = await db.query(
    `INSERT INTO civic_app.report_feedbacks (report_id, user_id, comment)
     VALUES ($1, $2, $3)
...
export async function flagReport(reportId, userId, { reason, details }) {
  const res = await db.query(
    `INSERT INTO civic_app.report_flags (report_id, user_id, reason, details)
     VALUES ($1, $2, $3, $4)
...
export async function updateReportStatus(reportId, { status, priority, notes }, reviewerId) {
  ...
  if (notes) {
    await db.query(
      `INSERT INTO civic_app.audit_events (table_name, record_id, user_id, action, changes)
...
export async function listReportsForAdmin(query) {
  ...
  const countSql = `SELECT COUNT(*) FROM civic_app.reports r ${whereClause}`;
...
export async function updateDispatch(dispatchId, { status, notes }, userId) {
  const res = await db.query(
    `UPDATE civic_app.report_authority_dispatches
     SET status = $2,
...
export async function getAuthoritiesNear(params) {
  ...
  const res = await db.query(
    `SELECT * FROM civic_app.find_authorities_nearby($1, $2, $3, $4, $5)`,
    [
      params.latitude,
      params.longitude,
      params.radius || 50000,
