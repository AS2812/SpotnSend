...
export async function updateSettings (userId, { language, theme, twoFactorPrompt }) {
  const res = await db.query(
  `INSERT INTO public.user_settings (user_id, language, theme, two_factor_prompt)
     VALUES ($1, COALESCE($2, 'en'), COALESCE($3, 'light'), COALESCE($4, FALSE))
     ON CONFLICT (user_id) DO UPDATE SET
  language = COALESCE(EXCLUDED.language, public.user_settings.language),
  theme = COALESCE(EXCLUDED.theme, public.user_settings.theme),
  two_factor_prompt = COALESCE(EXCLUDED.two_factor_prompt, public.user_settings.two_factor_prompt),
       updated_at = NOW()
...
export async function getAccountStats (userId) {
  ...
  `SELECT
    (SELECT COUNT(*) FROM public.reports WHERE user_id = $1) AS reports_count,
    (SELECT COUNT(*) FROM public.report_feedbacks WHERE user_id = $1) AS feedback_count,
    (SELECT account_status FROM public.users WHERE user_id = $1) AS account_status`,
  [userId]
  )
...
export async function updateNotificationPreferences (userId, prefs) {
  ...
  `INSERT INTO public.user_notification_preferences (
     ...
     ON CONFLICT (user_id) DO UPDATE SET
  notifications_enabled = COALESCE(EXCLUDED.notifications_enabled, public.user_notification_preferences.notifications_enabled),
  push_enabled = COALESCE(EXCLUDED.push_enabled, public.user_notification_preferences.push_enabled),
  email_enabled = COALESCE(EXCLUDED.email_enabled, public.user_notification_preferences.email_enabled),
  sms_enabled = COALESCE(EXCLUDED.sms_enabled, public.user_notification_preferences.sms_enabled),
        updated_at = NOW()
...
export async function listFavoriteSpots (userId) {
  ...
  `SELECT favorite_spot_id, name, latitude, longitude, radius_meters, created_at
   FROM public.favorite_spots WHERE user_id = $1 ORDER BY created_at DESC`,
  [userId]
  )
  return res.rows
}
...
export async function createFavoriteSpot (userId, { name, latitude, longitude, radiusMeters }) {
  const res = await db.query(
  `INSERT INTO public.favorite_spots (user_id, name, latitude, longitude, radius_meters)
     VALUES ($1, $2, $3, $4, $5)
...
export async function deleteFavoriteSpot (userId, spotId) {
  const res = await db.query(
    'DELETE FROM public.favorite_spots WHERE favorite_spot_id = $1 AND user_id = $2',
    [spotId, userId]
  )
...
export async function getMapPreferences (userId) {
  ...
  `SELECT default_radius_meters, default_view, include_favorites_by_default
   FROM public.user_map_preferences WHERE user_id = $1`,
  [userId]
  )
  return res.rows[0] || null
}
...
export async function updateMapPreferences (userId, prefs) {
  ...
  const res = await db.query(
  `INSERT INTO public.user_map_preferences (user_id, default_radius_meters, default_view, include_favorites_by_default)
     VALUES ($1, COALESCE($2, 1000), COALESCE($3, 'map'), COALESCE($4, TRUE))
     ON CONFLICT (user_id) DO UPDATE SET
  default_radius_meters = COALESCE(EXCLUDED.default_radius_meters, public.user_map_preferences.default_radius_meters),
  default_view = COALESCE(EXCLUDED.default_view, public.user_map_preferences.default_view),
  include_favorites_by_default = COALESCE(EXCLUDED.include_favorites_by_default, public.user_map_preferences.include_favorites_by_default),
       updated_at = NOW()
...
export async function setCategoryFilters (userId, categoryIds) {
  return withTransaction(async (client) => {
    await client.query('DELETE FROM public.user_category_filters WHERE user_id = $1', [userId])
    if (!categoryIds || categoryIds.length === 0) return []
    const values = categoryIds.map((_, index) => `($1, $${index + 2}, TRUE)`).join(', ')
    const params = [userId, ...categoryIds]
    await client.query(
  `INSERT INTO public.user_category_filters (user_id, category_id, is_selected)
       VALUES ${values}`,
  params
    )
    return categoryIds
...
export async function listCategoryFilters (userId) {
  const res = await db.query(
    'SELECT category_id, is_selected FROM public.user_category_filters WHERE user_id = $1',
    [userId]
  )
  return res.rows
}
